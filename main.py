...
def main():
    ...
    with c1:
        ...
        year = st.number_input("Enter the release year:", min_value=0, step=1, value=2000)
        ram = st.number_input("Enter the RAM of the mobile in MB:", min_value=0, step=1)
        sto = st.number_input("Enter the internal storage of the mobile:", min_value=0, step=1)
    ...
    #right column
    with c2:
        est = st.selectbox("Enter the type of expandable storage:", ['microSD', 'Nano Memory Card', 'SD card'])
        ...
        stype = st.selectbox("Enter the SIM type of the mobile:", ['Nano-SIM', 'Micro-SIM', 'Regular'])
        gscd = st.selectbox("Enter the GSM/CDMA type:", ['GSM', 'GSM + CDMA', 'CDMA'])
        bat = st.number_input("Enter the battery of the mobile:", min_value=0, step=1, value=4000)
        res = st.number_input("Enter the resolution of the mobile:", min_value=1080, step=1)
    ...
    data = [[brand,rcc,rc,fcc,fc,year,sc,os,sto,wf,blu,bat,ram,ts,ff,acc,ps,als,es,rf,pro,gps,res,promk,h,b,w,wt,cm,hp,gyro,est,g43,st,gscd,esl]]
    #submit button logic
    if st.button("Submit"):
